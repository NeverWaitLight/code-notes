...
class LightRAGClient:
    ...
    def query(
        ...
        参数说明
        question: 用户问题
        mode: 查询模式 支持 naive local global mix
        top_k: 检索的实体关系数量
        run_id: 运行标识符

        返回结果
        包含 answer 和 logs 的字典
        """
        if run_id is None:
        ...
        try:
            ...
            logs_path = self.export_traces_to_log(run_id, question, {"answer": answer})
            return {"answer": answer, "run_id": run_id, "logs": logs_path}
